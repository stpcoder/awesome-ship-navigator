#!/usr/bin/env python3
"""
Enhanced Collision Avoidance System V4
- Same logic as V3
- Includes path difference visualization at the end
"""
...
class ImprovedCostOptimizer:
    ...
    def optimize_ship(self, new_ship: ShipRoute, existing_ships: List[ShipRoute],
        ...
        """Find optimal departure time and path for new ship"""
...
def visualize_improved_collision_avoidance(ships: List[ShipRoute], collision_checker: CollisionChecker,
                                          obstacles_polygons: List[Polygon], background_image=None):
    """Visualize collision avoidance with improved metrics"""
    fig, axes = plt.subplots(2, 2, figsize=(20, 16))

    # Top-left: Paths overview
    ax1 = axes[0, 0]
    if background_image and os.path.exists(background_image):
        img = Image.open(background_image)
        ax1.imshow(img, extent=[0, img.width, img.height, 0], alpha=0.3)

    # Draw obstacles
    for poly in obstacles_polygons:
        x, y = poly.exterior.xy
        ax1.fill(x, y, 'red', alpha=0.3, edgecolor='darkred', linewidth=1)
    ...
    for ship in ships:
        if ship.path:
            ...
            ax1.plot(x_coords, y_coords, color=ship.color, linewidth=3, alpha=0.8,
                    label=f'{ship.name} (t={ship.departure_time:.0f}min, {ship.speed_knots}kn)')

            # Start and goal markers
            ax1.plot(ship.start[0], ship.start[1], 'o', color=ship.color, markersize=12,
                    markeredgecolor='black', markeredgewidth=2)
            ax1.plot(ship.goal[0], ship.goal[1], 's', color=ship.color, markersize=12,
                    markeredgecolor='black', markeredgewidth=2)

    ax1.set_title('Ship Paths with Improved Cost Function', fontsize=14)
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ax1.set_xlim(0, 2000)
    ax1.set_ylim(1400, 0)

    # Top-right: Time-space diagram
    ax2 = axes[0, 1]
    for ship in ships:
        if ship.timestamps:
            distances_nm = np.cumsum([0] + [
                np.sqrt((ship.path[i+1][0] - ship.path[i][0])**2 +
                       (ship.path[i+1][1] - ship.path[i][1])**2) * PIXEL_TO_NM
                for i in range(len(ship.path) - 1)
            ])
            ax2.plot(ship.timestamps, distances_nm, color=ship.color, linewidth=2,
                    marker='o', markersize=3, label=f'{ship.name} ({ship.speed_knots}kn)')

    ax2.set_xlabel('Time (minutes)')
    ax2.set_ylabel('Distance traveled (nautical miles)')
    ax2.set_title('Time-Space Diagram (Real Units)')
    ax2.legend()
    ...
    for ship in ships:
        if ship.path and ship.timestamps:
            ship_names.append(ship.name)
            speeds.append(ship.speed_knots)
            path_lengths_nm.append(ship.path_length_nm)
            travel_times.append(ship.timestamps[-1] - ship.departure_time)

    x = np.arange(len(ship_names))
    width = 0.35

    ax3_twin = ax3.twinx()
    bars1 = ax3.bar(x - width/2, speeds, width, label='Speed (knots)', color='skyblue')
    bars2 = ax3_twin.bar(x + width/2, travel_times, width, label='Travel Time (min)', color='coral')

    ax3.set_xlabel('Ships')
    ax3.set_ylabel('Speed (knots)', color='skyblue')
    ax3_twin.set_ylabel('Travel Time (minutes)', color='coral')
    ax3.set_title('Ship Speed and Travel Time Comparison')
    ax3.set_xticks(x)
    ax3.set_xticklabels(ship_names)
    ax3.legend(loc='upper left')
    ax3_twin.legend(loc='upper right')

    # Bottom-right: Summary with real units
    ax4 = axes[1, 1]
    ax4.axis('off')

    summary_text = "Navigation Summary (Real Units)\n" + "="*50 + "\n\n"
    for ship in ships:
        summary_text += f"{ship.name}:\n"
        summary_text += f"  Speed: {ship.speed_knots:.1f} knots\n"
        summary_text += f"  Departure: t={ship.departure_time:.1f} minutes\n"
        if ship.timestamps:
            summary_text += f"  Arrival: t={ship.timestamps[-1]:.1f} minutes\n"
            summary_text += f"  Travel time: {ship.timestamps[-1] - ship.departure_time:.1f} minutes\n"
        if ship.path:
            summary_text += f"  Path length: {ship.path_length_nm:.2f} nautical miles\n"
        summary_text += "\n"

    # Collision check
    summary_text += "Collision Status:\n"
    for i, ship1 in enumerate(ships):
        for j, ship2 in enumerate(ships):
            if i < j:
                collisions = collision_checker.check_collision(ship1, ship2)
                if collisions:
                    summary_text += f"  ⚠️ {ship1.name} - {ship2.name}: COLLISION\n"
                else:
                    summary_text += f"  ✅ {ship1.name} - {ship2.name}: Safe\n"

    # Add scale information
    summary_text += f"\nScale Information:\n"
    summary_text += f"  1 pixel = {PIXEL_TO_METERS:.1f} meters\n"
    summary_text += f"  Safety distance: {collision_checker.safety_distance_nm:.2f} nm\n"

    ax4.text(0.05, 0.95, summary_text, transform=ax4.transAxes,
            fontsize=10, verticalalignment='top', fontfamily='monospace')

    plt.tight_layout()
    plt.savefig('collision_avoidance_v4.png', dpi=150, bbox_inches='tight')
    print("\nVisualization saved: collision_avoidance_v4.png")


def visualize_path_comparison(ship_a: ShipRoute, ship_b: ShipRoute, ship_c: ShipRoute,
                             original_c_path: List[Tuple[float, float]],
                             path_adjuster: PathAdjuster, obstacles_polygons: List[Polygon]):
    """Compare original path vs alternative path for Ship C"""

    print("\n7. Creating path comparison visualization...")

    # Create visualization
    fig, axes = plt.subplots(2, 3, figsize=(18, 12))

    test_times = [0, 10, 15, 20, 30, 40]
    avoidance_radius_pixels = path_adjuster.collision_checker.safety_distance_pixels if hasattr(path_adjuster, 'collision_checker') else 92.6  # 0.5 nm in pixels

    existing_ships = [ship_a, ship_b]

    for idx, test_time in enumerate(test_times):
        ax = axes[idx // 3, idx % 3]

        # Draw obstacles
        for poly in obstacles_polygons:
            x, y = poly.exterior.xy
            ax.fill(x, y, 'lightgray', alpha=0.5, edgecolor='gray', linewidth=1)

        # Draw existing ship positions at test_time
        ship_a_pos = ship_a.get_position_at_time(test_time)
        ship_b_pos = ship_b.get_position_at_time(test_time)

        if ship_a_pos:
            circle_a = Circle(ship_a_pos, avoidance_radius_pixels, color='blue',
                            fill=False, linestyle='--', linewidth=2)
            ax.add_patch(circle_a)
            ax.plot(ship_a_pos[0], ship_a_pos[1], 'bo', markersize=8, label='Ship A')

        if ship_b_pos:
            circle_b = Circle(ship_b_pos, avoidance_radius_pixels, color='green',
                            fill=False, linestyle='--', linewidth=2)
            ax.add_patch(circle_b)
            ax.plot(ship_b_pos[0], ship_b_pos[1], 'go', markersize=8, label='Ship B')

        # Draw Ship A and B full paths
        if ship_a.path:
            ax.plot([p[0] for p in ship_a.path], [p[1] for p in ship_a.path],
                   'b-', alpha=0.3, linewidth=1)
        if ship_b.path:
            ax.plot([p[0] for p in ship_b.path], [p[1] for p in ship_b.path],
                   'g-', alpha=0.3, linewidth=1)

        # Draw original Ship C path
        if original_c_path:
            ax.plot([p[0] for p in original_c_path], [p[1] for p in original_c_path],
                   'r--', alpha=0.5, linewidth=2, label='C original')

        # Draw optimized Ship C path
        if ship_c.path:
            ax.plot([p[0] for p in ship_c.path], [p[1] for p in ship_c.path],
                   'darkred', linewidth=3, label='C optimized')

            # Show Ship C position at this time
            ship_c_pos = ship_c.get_position_at_time(test_time)
            if ship_c_pos:
                ax.plot(ship_c_pos[0], ship_c_pos[1], 'ro', markersize=8)

        # Draw start and goal
        ax.plot(ship_c.start[0], ship_c.start[1], 'r^', markersize=12,
               markeredgecolor='black', markeredgewidth=2)
        ax.plot(ship_c.goal[0], ship_c.goal[1], 'rs', markersize=12,
               markeredgecolor='black', markeredgewidth=2)

        ax.set_xlim(0, 2000)
        ax.set_ylim(1400, 0)
        ax.grid(True, alpha=0.3)
        ax.set_title(f't={test_time} min', fontsize=10)

        if idx == 0:
            ax.legend(loc='upper right', fontsize=8)

    plt.suptitle('Ship C Path Comparison: Original vs Optimized\n' +
                 f'Ship C departure: t={ship_c.departure_time:.0f} min (delayed {ship_c.departure_time:.0f} min)',
                 fontsize=14)
    plt.tight_layout()
    plt.savefig('path_comparison_v4.png', dpi=150, bbox_inches='tight')
    print("   Path comparison saved: path_comparison_v4.png")
    ...
    for poly in obstacles_polygons:
        x, y = poly.exterior.xy
        ax2.fill(x, y, 'lightgray', alpha=0.5, edgecolor='gray', linewidth=1)

    # Original collision time (before optimization)
    collision_time = 15.75  # Middle of original collision interval [14.5, 17.0]

    # Get ship positions at original collision time
    ship_a_collision_pos = ship_a.get_position_at_time(collision_time)

    # Draw avoidance zones
    if ship_a_collision_pos:
        for radius in [50, 75, 100]:
            circle = Circle(ship_a_collision_pos, radius, color='blue',
                          fill=False, alpha=0.3 + radius/300, linewidth=1)
            ax2.add_patch(circle)
        ax2.plot(ship_a_collision_pos[0], ship_a_collision_pos[1], 'bo',
                markersize=10, label=f'Ship A @ t={collision_time}')

    # Draw full paths
    ax2.plot([p[0] for p in ship_a.path], [p[1] for p in ship_a.path],
            'b-', alpha=0.3, linewidth=2, label='Ship A path')
    ax2.plot([p[0] for p in ship_b.path], [p[1] for p in ship_b.path],
            'g-', alpha=0.3, linewidth=2, label='Ship B path')

    # Draw original Ship C path
    if original_c_path:
        ax2.plot([p[0] for p in original_c_path], [p[1] for p in original_c_path],
                'r--', alpha=0.5, linewidth=3, label='Ship C original path')

        # Show where collision would have occurred
        temp_ship_c = ShipRoute(
            name="Ship C",
            start=ship_c.start,
            goal=ship_c.goal,
            path=original_c_path,
            departure_time=0,
            speed_knots=ship_c.speed_knots,
            color='red'
        )
        temp_ship_c.path_length_pixels = sum(
            np.sqrt((original_c_path[i+1][0] - original_c_path[i][0])**2 +
                   (original_c_path[i+1][1] - original_c_path[i][1])**2)
            for i in range(len(original_c_path) - 1)
        )
        temp_ship_c.calculate_timestamps()

        ship_c_collision_pos = temp_ship_c.get_position_at_time(collision_time)
        if ship_c_collision_pos and ship_a_collision_pos:
            ax2.plot(ship_c_collision_pos[0], ship_c_collision_pos[1],
                    'rx', markersize=15, markeredgewidth=3,
                    label=f'Collision point @ t={collision_time}')

            # Show collision distance
            distance = np.sqrt(
                (ship_c_collision_pos[0] - ship_a_collision_pos[0])**2 +
                (ship_c_collision_pos[1] - ship_a_collision_pos[1])**2
            )
            ax2.plot([ship_c_collision_pos[0], ship_a_collision_pos[0]],
                    [ship_c_collision_pos[1], ship_a_collision_pos[1]],
                    'r-', linewidth=2, alpha=0.7)
            mid_x = (ship_c_collision_pos[0] + ship_a_collision_pos[0]) / 2
            mid_y = (ship_c_collision_pos[1] + ship_a_collision_pos[1]) / 2
            ax2.text(mid_x, mid_y, f'{distance:.0f} px\n({distance * PIXEL_TO_NM:.2f} nm)',
                    fontsize=12, color='red', weight='bold', ha='center',
                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

    # Draw optimized path
    if ship_c.path:
        ax2.plot([p[0] for p in ship_c.path], [p[1] for p in ship_c.path],
                'darkred', linewidth=4, label=f'Ship C optimized (t={ship_c.departure_time:.0f})')

    # Start and goal
    ax2.plot(ship_c.start[0], ship_c.start[1], 'r^', markersize=15,
            markeredgecolor='black', markeredgewidth=2)
    ax2.text(ship_c.start[0], ship_c.start[1] - 40, 'Ship C Start',
            ha='center', fontsize=10, weight='bold')

    ax2.plot(ship_c.goal[0], ship_c.goal[1], 'rs', markersize=15,
            markeredgecolor='black', markeredgewidth=2)
    ax2.text(ship_c.goal[0], ship_c.goal[1] - 40, 'Ship C Goal',
            ha='center', fontsize=10, weight='bold')
    ...
    ax2.set_title(f'Collision Avoidance Detail\n' +
                  f'Original collision would occur at t={collision_time:.1f} min\n' +
                  f'Solution: Delay departure by {ship_c.departure_time:.0f} min',
                  fontsize=14)

    plt.tight_layout()
    plt.savefig('collision_detail_v4.png', dpi=150, bbox_inches='tight')
    print("   Collision detail saved: collision_detail_v4.png")


def main():
    print("="*60)
    print("Collision Avoidance V4 - With Path Comparison")
    print("="*60)
    print(f"\nScale Configuration:")
    ...
    # 4. Optimize with improved cost function
    print("\n4. Optimizing with improved cost function...")
    ...
    # 6. Visualize results
    print("\n6. Creating main visualization...")
    visualize_improved_collision_avoidance(all_ships, collision_checker, obstacles_polygons, 'nn.png')

    # 7. Visualize path comparison
    visualize_path_comparison(ship_a, ship_b, ship_c, original_c_path, path_adjuster, obstacles_polygons)

    print("\n" + "="*60)
    print("Complete!")
    print("Visualizations created:")
    print("  - collision_avoidance_v4.png (main results)")
    print("  - path_comparison_v4.png (path comparison over time)")
    print("  - collision_detail_v4.png (collision scenario detail)")
    print("="*60)
